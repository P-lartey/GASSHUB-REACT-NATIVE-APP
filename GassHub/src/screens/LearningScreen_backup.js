...
const LearningScreen = ({navigation, onBackPress}) => {
  ...
  useEffect(() => {
    if (!materialsLoading && courseMaterials && courseMaterials.length > 0) {
      ...
      courseMaterials.forEach(async (material) => {
        try {
          const storageFiles = await getCourseMaterialsFromStorage(material.id);
          ...
          console.log(`No storage materials found for ${material.id}`);
        }
      });
    }
  ...
  // Combine Firestore files with Storage files
  const materialsWithDefaults = effectiveCourseMaterials.map(material => {
    // Get storage files for this material
    const storageFiles = storageMaterials[material.id] || [];
    
    // Combine Firestore files with Storage files
    // Handle nested files structure from Firebase
    let firestoreFiles = [];
    if (material.files && typeof material.files === 'object') {
      // Convert the nested file objects (file1, file2, etc.) to an array
      Object.keys(material.files).forEach(key => {
        const file = material.files[key];
        if (file && file.fileUrl) {
          firestoreFiles.push({
            name: file.name || key,
            url: file.fileUrl,
            type: file.type || 'slides',
            fileType: getFileExtension(file.fileUrl),
          });
        }
      });
    } else if (material.fileUrl) {
      // Fallback to single fileUrl if present
      firestoreFiles = [
        {
          name: material.name || material.code || 'Course Material',
          url: material.fileUrl,
          type: 'file',
          fileType: getFileExtension(material.fileUrl),
        }
      ];
    } else if (material.files && Array.isArray(material.files)) {
      firestoreFiles = material.files;
    }
    ...
    return {
      ...material,
      title: material.name || material.code || 'Untitled', // Map name to title
      rating: typeof material.rating !== 'undefined' ? material.rating : 0, // Use 0 if not provided
  ...
  const learningData = {
    '100': {
      first: materialsWithDefaults.filter(material => material.level === '100' && material.semester === 'first'),
      second: materialsWithDefaults.filter(material => material.level === '100' && material.semester === 'second'),
    },
    '200': {
      first: materialsWithDefaults.filter(material => material.level === '200' && material.semester === 'first'),
      second: materialsWithDefaults.filter(material => material.level === '200' && material.semester === 'second'),
    },
    '300': {
      first: materialsWithDefaults.filter(material => material.level === '300' && material.semester === 'first'),
      second: materialsWithDefaults.filter(material => material.level === '300' && material.semester === 'second'),
    },
    '400': {
      first: materialsWithDefaults.filter(material => material.level === '400' && material.semester === 'first'),
      second: materialsWithDefaults.filter(material => material.level === '400' && material.semester === 'second'),
    },
  };
  ...
  // Pulsing animation for the AI button
  useEffect(() => {
  ...
  const debugFileUrl = (file) => {
    ...
    console.log('File type:', getFileExtension(fileUrl));
    ...
    <View style={styles.header}>
      {showBackButton && (
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.backButtonText, {color: '#1E3A8A'}]}>◄</Text>
        </TouchableOpacity>
      )}
      <Text style={[styles.headerTitle, {color: '#1E3A8A', marginLeft: 20}]}>{title}</Text>
      <View style={styles.headerRight}>
        <TouchableOpacity 
          ...
          activeOpacity={0.7}
        >
          <Text style={[styles.themeIcon, {color: '#1E3A8A'}]}>{isDarkMode ? '🌙' : '☀️'}</Text>
        </TouchableOpacity>
        <View style={[styles.profileButton, {backgroundColor: '#1E3A8A'}]}>
          <Text style={[styles.profileEmoji, {color: '#FFFFFF'}]}>👤</Text>
        </View>
      </View>
    </View>
  );
  ...
  const renderFilePreview = (file) => {
    ...
    // Auto-preview implementation for PDF files
    if (file.isPdf) {
      const originalFileUrl = file.url || file.previewUrl;
      
      // Process Firebase Storage URLs for preview
      let previewUrl = originalFileUrl;
      console.log('Original preview URL:', originalFileUrl);
      
      if (originalFileUrl && originalFileUrl.includes('firebasestorage.googleapis.com')) {
        try {
          const urlObj = new URL(originalFileUrl);
          // Log original parameters
          console.log('Original Firebase URL params:', Object.fromEntries(urlObj.searchParams));
          
          // Remove parameters that force downloads
          urlObj.searchParams.delete('alt');
          urlObj.searchParams.delete('download');
          urlObj.searchParams.delete('response-content-disposition');
          
          // Add inline viewing parameter
          urlObj.searchParams.set('alt', 'media');
          previewUrl = urlObj.toString();
          
          console.log('Processed preview URL:', previewUrl);
          console.log('Processed URL params:', Object.fromEntries(new URL(previewUrl).searchParams));
        } catch (urlError) {
          console.log('Preview URL processing error:', urlError);
          previewUrl = originalFileUrl; // Fallback to original
        }
      } else {
        console.log('Using original URL for preview:', originalFileUrl);
        previewUrl = originalFileUrl;
      }
      
      return (
        <View style={styles.previewContainer}>
          <View style={styles.autoPreviewHeader}>
            <Text style={[styles.previewBadge, {backgroundColor: colors.primary}]}>Preview</Text>
          </View>
          ...
          <View style={styles.pdfPreviewContainer}>
            {isLoading && (
              <View style={styles.loadingOverlay}>
                ...
                <Text style={[styles.loadingText, {color: colors.textSecondary}]}>Loading preview...</Text>
              </View>
            )}
            
            {/* PDF Auto Preview using Google Docs Viewer */}
            <WebView
              source={{
                uri: `https://docs.google.com/gviewer?url=${encodeURIComponent(previewUrl)}&embedded=true&widget=true&headers=false`
              }}
              style={styles.pdfPreviewFrame}
              javaScriptEnabled={true}
              domStorageEnabled={true}
              startInLoadingState={true}
              onLoad={() => setPreviewLoading(prev => ({...prev, [fileId]: false}))}
              onError={() => setPreviewLoading(prev => ({...prev, [fileId]: false}))}
              ...
              renderError={(errorDomain, errorCode, errorDesc) => (
                <View style={[styles.genericPreview, { backgroundColor: isDarkMode ? colors.surface : '#F1F5F9' }]}> 
                  <Text style={{ color: colors.text, fontSize: 16, textAlign: 'center' }}>
                    ⚠️ Preview unavailable
                  </Text>
                  <Text style={{ color: colors.textSecondary, fontSize: 12, textAlign: 'center', marginTop: 5 }}>
                    Click "Open Full" to view
                  </Text>
                </View>
              )}
            />
          ...
          <View style={styles.previewInfo}>
            <Text style={[styles.previewInfoText, {color: colors.textSecondary}]}>Page 1 • Click "Open Full" to see all pages</Text>
          </View>
        </View>
      );
    }
    ...
    if (file.isImage) {
      return (
        <View style={styles.previewContainer}>
          ...
          <Image 
            source={{uri: file.url || file.previewUrl}} 
    ...
    if (file.isText) {
      return (
        <View style={styles.previewContainer}>
          ...
          <View style={[styles.textPreview, {backgroundColor: isDarkMode ? '#334155' : '#F0F9FF'}]}>
    ...
    return (
      <View style={styles.previewContainer}>
        ...
        <View style={[styles.genericPreview, {backgroundColor: isDarkMode ? '#334155' : '#E2E8F0'}]}>
          <Text style={{fontSize: 24, color: isDarkMode ? '#94A3B8' : '#64748B'}}>
            {getFileIcon(file.type)}
          </Text>
          <Text style={{fontSize: 14, color: isDarkMode ? '#94A3B8' : '#64748B', marginTop: 8}}>
    ...
    <View key={file.name} style={[styles.fileCard, {backgroundColor: isDarkMode ? colors.surface : '#EBF8FF', borderColor: colors.borderLight}]}>      
      ...
      <View style={styles.fileHeader}>
        ...
        <View style={styles.fileInfo}>
          <Text style={[styles.fileTitle, {color: colors.text}]}>{file.name}</Text>
          <Text style={[styles.fileMetadata, {color: colors.textSecondary}]}>            
            {getFileTypeName(file.type)} • {file.size} {file.pageCount ? `• ${file.pageCount} pages` : ''} {file.duration ? `• ${file.duration}` : ''}
          </Text>
        </View>
      ...
      <View style={styles.fileActions}>
        ...
        >
          <Text style={styles.actionButtonText}>📖 Open Full</Text>
        </TouchableOpacity>
        <TouchableOpacity 
      ...
      >
        ...
        >
          ...
          <View style={styles.aiButtonGradient}>
            <View>
              <Text style={styles.aiSearchButtonTextShadow}>🤖 Hi I'm XtraAI</Text>
              <Text style={styles.aiSearchButtonText}>🤖 Hi I'm XtraAI</Text>
            </View>
          </View>
        </TouchableOpacity>
        </Animated.View>
    ...
    return (
      <View style={[styles.container, {backgroundColor: colors.background}]}>
        ...
        <View style={[styles.header, {backgroundColor: '#1E3A8A'}]}>
          ...
          <Text style={[styles.headerTitle, {color: '#FFFFFF', marginLeft: 20}]}>Level {selectedLevel}</Text>
          <View style={styles.headerRight}>
            <TouchableOpacity 
              style={styles.themeToggle} 
              onPress={toggleTheme}
              activeOpacity={0.7}
            >
              <Text style={[styles.themeIcon, {color: '#FFFFFF'}]}>{isDarkMode ? '🌙' : '☀️'}</Text>
            </TouchableOpacity>
            <View style={[styles.profileButton, {backgroundColor: '#1E3A8A'}]}>
              <Text style={[styles.profileEmoji, {color: '#FFFFFF'}]}>👤</Text>
            </View>
      ...
      >
        <View style={{ flex: 1, backgroundColor: colors.background }}>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 10, paddingTop: 40, backgroundColor: colors.background }}>
            <Text style={[styles.headerTitle, { color: colors.text }]}>Document Viewer</Text>
            <TouchableOpacity 
              style={{ padding: 8 }}
              onPress={() => {
                setWebViewModalVisible(false);
...
const styles = StyleSheet.create({
  ...
  backButton: {
    padding: 8,
    marginLeft: 10, // Move the back button to the left corner a bit
  },
  backButtonText: {
    fontSize: 24,
    fontWeight: 'bold',
  ...
  levelSelectionContent: {
    padding: 20,
    paddingTop: 30, // Adjusted for the new background position
  },
  welcomeTitle: {
    fontSize: 32,
    fontWeight: 'bold',
    marginBottom: 10,
    textAlign: 'center',
    marginTop: 30, // Move the title down a bit
  },
  welcomeSubtitle: {
    fontSize: 16,
    textAlign: 'center',
